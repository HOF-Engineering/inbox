...
RSpec.describe ConversationFinder do
  describe '#perform' do
    it 'excludes participant-only conversations assigned to another agent for participating custom roles' do
      ...
      create(:conversation_participant, account: account, conversation: own_conversation, user: agent)
      ...
      expect(result[:conversations].map(&:id)).to contain_exactly(own_conversation.id)
    end
  end
end
